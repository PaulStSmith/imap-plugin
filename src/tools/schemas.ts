...
export const addAccountSchema = z.object({
  ...
  credentialProvider: z.enum(["local-keychain", "1password", "env", "sql-vault", "dev-sql-vault"]).default("local-keychain"),
  credentialRef: z.string().optional().describe("1Password op:// reference, environment variable name, or SQL vault secret reference."),
  password: z.string().optional().describe("Password to store when using local-keychain or a SQL vault."),
